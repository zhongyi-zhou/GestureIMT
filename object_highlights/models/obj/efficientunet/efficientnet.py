...
class EfficientNet(nn.Module):
    ...
    def __init__(self, block_args_list, global_params, in_channels=3):
        ...
        # in_channels_add = 1
        # self._conv_stem_add = Conv2dSamePadding(in_channels_add,
        #                                     out_channels,
        #                                     kernel_size=3,
        #                                     stride=2,
        #                                     bias=False,
        #                                     name='stem_conv')
        # self._bn0_add = BatchNorm2d(num_features=out_channels,
        #                         momentum=batch_norm_momentum,
        #                         eps=batch_norm_epsilon,
        #                         name='stem_batch_norm')

        # self._swish_add = Swish(name='swish')
    ...
    @classmethod
    def from_name(cls, model_name, *, in_channels=3, n_classes=1000, pretrained=False):
        return _get_model_by_name(model_name, in_channels=in_channels, classes=n_classes, pretrained=pretrained)

    @classmethod
    def encoder(cls, model_name, *, in_channels=3, pretrained=False):
        model = cls.from_name(model_name, in_channels=in_channels, pretrained=pretrained)
        ...
        class Encoder(nn.Module):
            def __init__(self):
                ...
                # self.stem_conv_add = model._conv_stem_add
                # self.stem_batch_norm_add = model._bn0_add
                # self.stem_swish_add = Swish(name='stem_swish')
            ...
            def forward(self, x):
                ...
                # x2 = self.stem_conv_add(x2)
                # x2 = self.stem_batch_norm_add(x2)
                # x2 = self.stem_swish_add(x2)
...
def _get_model_by_name(model_name, in_channels=3, classes=1000, pretrained=False):
